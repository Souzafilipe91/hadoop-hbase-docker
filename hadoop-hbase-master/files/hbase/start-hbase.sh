...
winpty docker exec -it "$hadoop_master" bash -c "cd $hbase_home/hbase && ./start-hbase.sh"

echo -e "starting hbase on slaves \n"
winpty docker exec -it "$hadoop_slave" bash -c "cd $hbase_home/hbase && ./start-hbase.sh"
